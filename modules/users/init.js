			...
			return `
				<div class="user-card" data-user-id="${htmlText(user.user_id)}">
					...
					<div class="user-card-actions">
						...
						<button class="card-btn" data-action="reportes" data-user-id="${htmlText(user.user_id)}">
							<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
			...
			document.addEventListener('click', async (e) => {
				...
				switch (action) {
					...
					case 'permissions':
						...
						break;
					case 'reportes':
						// Mostrar reportes asignados a este usuario
						await this.showUserReportesModal(userId);
						break;
				}
			});
		}

		async showUserReportesModal(userId) {
			const supabase = window.App?.supabase;
			if(!supabase){ window.UI?.toast('Supabase no disponible','error'); return; }
			// Buscar reportes asignados a este usuario usando RPC
			const { data: reportes, error } = await supabase.rpc('get_reportes_by_user', { p_user_id: userId, p_limit: 30 });
			let html = `<div style='min-width:340px;max-width:90vw;'>
				<h3>Reportes asignados</h3>
				<ul style='max-height:300px;overflow:auto;padding-left:0;'>`;
			if(error || !reportes){
				html += `<li style='color:#888;'>No tiene reportes asignados</li>`;
			} else if(reportes.length){
				html += reportes.map(r=>`<li style='margin-bottom:8px;list-style:none;'>
					<a href=\"#/reportes/${r.id}\" class=\"rep-link\" style=\"font-weight:bold;\">${htmlText(r.titulo||'Reporte #'+r.id)}</a>
					<span style=\"margin-left:8px;font-size:0.9em;color:#888;\">${htmlText(r.estado||'pendiente')}</span>
					<span style=\"margin-left:8px;font-size:0.9em;color:#888;\">${new Date(r.created_at).toLocaleDateString()}</span>
				</li>`).join('');
			} else {
				html += `<li style='color:#888;'>No tiene reportes asignados</li>`;
			}
			html += `</ul>
				<div style='text-align:right;margin-top:12px;'><button class='ui-btn' id='close-reportes-modal'>Cerrar</button></div>
			</div>`;
			const modal = window.UI?.modal(html);
			setTimeout(()=>{
				document.getElementById('close-reportes-modal')?.addEventListener('click', ()=>modal.close());
			},100);
		}
		...
		async openRolePermissions(roleKey){
			...
			if(!saveBtn.dataset.bound){
				saveBtn.addEventListener('click', async () => {
					saveBtn.disabled = true;
					try{
						// Obtener el roleKey actual del input oculto (evita problemas de clausura)
						const currentRoleKey = document.getElementById('current-role-key').value;
						if(!currentRoleKey) throw new Error('No se ha seleccionado ningún rol');
						...
						alert(`Permisos del rol "${currentRoleKey}" actualizados correctamente. Se asignaron ${selected.length} permisos.`);
						this.app.managers.modals.closeModal('role-permissions-modal');
						
						// Recargar datos para reflejar cambios en la UI principal
						await this.app.loadRolePermissions();
						this.app.managers.roles.render();
