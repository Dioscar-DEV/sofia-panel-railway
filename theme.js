...
(function(){
  ...
  const DEFAULT_THEME = {
    ...
    bannerText: 'Sistema Agéntico',
    footer: {
      text: '© 2025 Fibex Telecom. Todos los derechos reservados.',
      links: [
    ...
    colors: {
      ...
      text: '#0f172a',
      muted: '#64748b',
      brand: '#0b1739',           // Color principal Fibex Telecom
      accent: '#0a1228',          // Color oscuro para acentos
      danger: '#dc2626',          // Rojo peligro
      success: '#10b981',         // Verde éxito
      warning: '#f59e0b',         // Naranja advertencia
      info: '#0ea5e9',            // Azul info
      brandLight: '#1a2d5f',      // Color claro derivado de brand
  ...
  function updateDOMWithTheme(theme) {
    ...
    // También actualizar por ID como fallback
    const brandNameEl = document.getElementById('brand-name');
    if (brandNameEl) brandNameEl.textContent = theme.brandName;
    ...
    // También actualizar logo por ID
    ...
    // También actualizar subtitle por ID
    const subtitleEl = document.getElementById('brand-subtitle');
    if (subtitleEl) subtitleEl.textContent = theme.bannerText;

    // Actualizar título de la página
    if (theme.brandName) {
      document.title = `${theme.brandName} - ${theme.bannerText || 'Sistema Agéntico'}`;
    }
